...
module.exports = {
    ...
    async run({ client, message, args }) {
        ...
        if (settings.dmMessages === true){
            üye.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle, ${time} boyunca mutelendiniz!`}).catch(() => {})
        }
        const penal = await client.penalize(message.guild.id, üye.user.id, "CHAT-MUTE", true, message.author.id, reason, true, Date.now() + duration);
  
        const embed = new EmbedBuilder()
        .setAuthor({name :  message.member.user.username , iconURL :  message.member.user.displayAvatarURL({dynamic: true , size : 4096})})
        .setDescription(`
        ${üye} (${üye.id}) üyesi sunucudan **${reason}** sebebiyle <t:${parseInt(Date.now() / 1000)}:F> tarihinde **mutelendi**. Ceza numarası : **#${penal.id}**`)
        .setFooter({text :  message.guild.name , iconURL :  message.guild.iconURL({dynamic: true , size : 4096})})
        .setColor(client.redColor())
        ...
        await ceza.findOneAndUpdate({ guildID: message.guild.id, userID: message.member.id }, { $inc: { MuteAmount: 1 } }, {upsert: true});
        ...
        if(kanallar.cezaLog.CezaPuan) 
        {
            ...
            .send({ content:`${üye} üyesi **mute** cezası alarak toplam \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanına\` ulaştı!`})
        }
