...
module.exports = {
    ...
    async run({ client, message, args }) {
        ...
        if (settings.dmMessages === true){
            üye.send({ content:`**${message.guild.name}** sunucusundan, **${message.author.tag}** tarafından, **${reason}** sebebiyle, ${time} süreyiyle jaillendiniz!`}).catch(() => {})
        }
        const penal = await client.penalize(message.guild.id, üye.user.id, "TEMP-JAIL", true, message.author.id, reason, true, Date.now() + duration);
