...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription("Bir Kullanıcıyı Süreli Olarak Jaillersiniz.")
    ...
    async run(interaction) {
        ...
        const data = await penals.findOne({ userID: üye.id, guildID: interaction.guild.id, $or: [{ type: "JAIL" }, { type: "TEMP-JAIL" }], active: true });
        if(üye.roles.cache.has(rol.roller.JailRole) || data ){ 
        const embed = new EmbedBuilder().setAuthor({name :  interaction.member.user.username , iconURL :  interaction.member.user.displayAvatarURL({dynamic: true , size : 4096})}).setDescription(`${üye} Kullanıcısının aktif **jail** cezası bulunmakta.`).setFooter({text :  interaction.guild.name , iconURL :  interaction.guild.iconURL({dynamic: true , size : 4096})}).setColor(client.redColor()) 
        await interaction.reply({embeds : [embed], ephemeral : true})
        return
    }
        ...
        const penal = await client.penalize(interaction.guild.id, üye.user.id, "TEMP-JAIL", true, interaction.member.id, reason, true, Date.now() + duration);

        const time = ms(duration).replace("h", " saat").replace("m", " dakika").replace("s", " saniye").replace("w", " hafta");
        if (settings.dmMessages === true){
            üye.send({ content:`**${interaction.guild.name}** sunucusundan, **${interaction.member.user.tag}** tarafından, **${reason}** sebebiyle, **${time}** boyunca jaillendiniz. Ceza numarası : **#${penal.id}**`}).catch(() => {})
        }
          
        const embed = new EmbedBuilder()
        .setAuthor({name :  interaction.member.user.username , iconURL :  interaction.member.user.displayAvatarURL({dynamic: true , size : 4096})})
        .setDescription(`
        ${üye} (${üye.id}) üyesi sunucuda **${reason}** sebebiyle <t:${parseInt(Date.now() / 1000)}:F> tarihinde **${time}** süreyiyle **jaillendi**. Ceza numarası : **#${penal.id}**`)
        .setFooter({text :  interaction.guild.name , iconURL :  interaction.guild.iconURL({dynamic: true , size : 4096})})
        .setColor(client.redColor())
        ...
        await ceza.findOneAndUpdate({ guildID: interaction.guild.id, userID: interaction.member.id }, { $inc: { JailAmount: 1 } }, {upsert: true});
        await cezapuan.findOneAndUpdate({ guildID: interaction.guild.id, userID: üye.user.id }, { $inc: { cezapuan: 20 } }, { upsert: true });
        await isimler.findOneAndUpdate({ guildID: interaction.guild.id, userID: üye.user.id }, { $push: { names: { name: üye.displayName, yetkili: interaction.member.id, rol: "Jail", date: Date.now() } } }, { upsert: true });
