...
module.exports = {
        ...
        .setDescription("Bir Kullanıcıyı Süreli Olarak Chat Kanallarından Mutelersiniz..")
        ...
        const data = await penals.findOne({ userID: üye.id, guildID: interaction.guild.id, $or: [{ type: "CHAT-MUTE" }], active: true });
        if(üye.roles.cache.has(rol.roller.ChatMuteRole) || data ){ 
        const embed = new EmbedBuilder().setAuthor({name :  interaction.member.user.username , iconURL :  interaction.member.user.displayAvatarURL({dynamic: true , size : 4096})}).setDescription(`${üye} Kullanıcısının aktif **CHAT-MUTE** cezası bulunmakta.`).setFooter({text :  interaction.guild.name , iconURL :  interaction.guild.iconURL({dynamic: true , size : 4096})}).setColor(client.redColor()) 
        await interaction.reply({embeds : [embed], ephemeral : true})
        return
        }
        ...
        await üye.roles.add(roller.roller.ChatMuteRole)
        const penal = await client.penalize(interaction.guild.id, üye.user.id, "CHAT-MUTE", true, interaction.member.id, reason, true, Date.now() + duration);
        const time = ms(duration).replace("h", " saat").replace("m", " dakika").replace("s", " saniye").replace("w", " hafta");
        if (settings.dmMessages === true){
        üye.send({ content:`**${interaction.guild.name}** sunucusundan, **${interaction.member.user.tag}** tarafından, **${reason}** sebebiyle, **${time}** boyunca **CHAT-MUTE** cezası aldınız. Ceza numarası : **#${penal.id}**`}).catch(() => {})
        }
          
        const embed = new EmbedBuilder()
        .setAuthor({name :  interaction.member.user.username , iconURL :  interaction.member.user.displayAvatarURL({dynamic: true , size : 4096})})
        .setDescription(`
        ${üye} (${üye.id}) üyesi sunucuda **${reason}** sebebiyle <t:${parseInt(Date.now() / 1000)}:F> tarihinde **${time}** süreyiyle **CHAT-MUTE** ceazası verildi. Ceza numarası : **#${penal.id}**`)
        .setFooter({text :  interaction.guild.name , iconURL :  interaction.guild.iconURL({dynamic: true , size : 4096})})
        .setColor(client.redColor())
        ...
        await ceza.findOneAndUpdate({ guildID: interaction.guild.id, userID: interaction.member.id }, { $inc: { MuteAmount: 1 } }, {upsert: true});
        await cezapuan.findOneAndUpdate({ guildID: interaction.guild.id, userID: üye.user.id }, { $inc: { cezapuan: 20 } }, { upsert: true });
        const cezapuanData = await cezapuan.findOne({ guildID: interaction.guild.id, userID: üye.user.id });    
        if(kanallar.cezaLog.CezaPuan) {interaction.guild.channels.cache.get(kanallar.cezaLog.CezaPuan).send({ content:`${üye} üyesi **CHAT-MUTE** cezası alarak toplam \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanına\` ulaştı!`})}
