...
module.exports = {
        ...
        .setDescription("Bir Kullanıcıyı Süreli Olarak Sesli Ve Chat Kanallarında Mutelersiniz..")
        ...
        const data = await penals.findOne({ userID: üye.id, guildID: interaction.guild.id, $or: [{ type: "TIME-OUT" }], active: true });
        if(üye.roles.cache.has(rol.roller.TimeOutRole) || data ){ 
        const embed = new EmbedBuilder().setAuthor({name :  interaction.member.user.username , iconURL :  interaction.member.user.displayAvatarURL({dynamic: true , size : 4096})}).setDescription(`${üye} Kullanıcısının aktif **TIME-OUT** cezası bulunmakta.`).setFooter({text :  interaction.guild.name , iconURL :  interaction.guild.iconURL({dynamic: true , size : 4096})}).setColor(client.redColor()) 
        await interaction.reply({embeds : [embed], ephemeral : true})
        return
        }
        ...
        await üye.timeout(duration)
        await üye.roles.add(roller.roller.TimeOutRole)
        const penal = await client.penalize(interaction.guild.id, üye.user.id, "TIME-OUT", true, interaction.member.id, reason, true, Date.now() + duration);
        const time = ms(duration).replace("h", " saat").replace("m", " dakika").replace("s", " saniye").replace("w", " hafta");
        if (settings.dmMessages === true){
        üye.send({ content:`**${interaction.guild.name}** sunucusundan, **${interaction.member.user.tag}** tarafından, **${reason}** sebebiyle, **${time}** boyunca **TIME-OUT** cezası aldınız. Ceza numarası : **#${penal.id}**`}).catch(() => {})
        }
          
        const embed = new EmbedBuilder()
        .setAuthor({name :  interaction.member.user.username , iconURL :  interaction.member.user.displayAvatarURL({dynamic: true , size : 4096})})
        .setDescription(`
        ${üye} (${üye.id}) üyesi sunucuda **${reason}** sebebiyle <t:${parseInt(Date.now() / 1000)}:F> tarihinde **${time}** boyunca **TIME-OUT** ceazası verildi. Ceza numarası : **#${penal.id}**`)
        .setFooter({text :  interaction.guild.name , iconURL :  interaction.guild.iconURL({dynamic: true , size : 4096})})
        .setColor(client.redColor())
        ...
        await ceza.findOneAndUpdate({ guildID: interaction.guild.id, userID: interaction.member.id }, { $inc: { TimeOutAmount: 1 } }, {upsert: true});
        await ceza.findOneAndUpdate({ guildID: interaction.guild.id, userID: üye.user.id }, { $inc: { top: 1 } }, { upsert: true });
        await cezapuan.findOneAndUpdate({ guildID: interaction.guild.id, userID: üye.user.id }, { $inc: { cezapuan: 40 } }, { upsert: true });   
        const cezapuanData = await cezapuan.findOne({ guildID: interaction.guild.id, userID: üye.user.id });    
        if(kanallar.cezaLog.CezaPuan) {interaction.guild.channels.cache.get(kanallar.cezaLog.CezaPuan).send({ content:`${üye} üyesi **TIME-OUT** cezası alarak toplam \`${cezapuanData ? cezapuanData.cezapuan : 0} ceza puanına\` ulaştı!`})}
